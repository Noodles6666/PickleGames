...
public class BilliardBall
{
    ...
    public boolean tick() {
        ...
        if (this.v_progress >= length || length < BilliardBall.BALL_DIAMETER) {
            // 直接使用 teleportAsync，这是 Folia 要求的
            final Location targetLoc = this.as.getLocation().add(this.v);
            if (SchedulerUtil.isFolia()) {
                this.as.teleportAsync(targetLoc);
            } else {
                this.as.teleport(targetLoc);
            }
    ...
    public boolean collide(final BilliardBall billiardBall) {
        ...
        if (this.v_progress > 0.0) {
            // 直接使用 teleportAsync
            if (SchedulerUtil.isFolia()) {
                this.as.teleportAsync(this.v_prog_loc);
            } else {
    ...
    public void sink() {
        ...
        this.sunk = true;
        this.v = new Vector(0, 0, 0);
        // 在 Folia 上需要在实体所在区域的线程中删除实体
        if (SchedulerUtil.isFolia()) {
            this.as.getScheduler().run(this.game.getPlugin(), task -> this.as.remove(), null);
        } else {
            this.as.remove();
        }
    }
    ...
    public void setLocation(final Location location) {
        final Location targetLoc = location.add(this.as_offset);
        if (SchedulerUtil.isFolia()) {
            this.as.teleportAsync(targetLoc);
        } else {
            this.as.teleport(targetLoc);
