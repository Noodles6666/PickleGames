    ...
    public boolean onCommand(final CommandSender commandSender, final Command command, final String s, final String[] array) {
        ...
        if (array[0].equalsIgnoreCase("help") || array[0].equalsIgnoreCase("?")) {
            if (player.hasPermission("lobbygames.command") || player.hasPermission("lobbygames.admin") || player.hasPermission("lobbygames.kickplayer")) {
                ...
                player.sendMessage(this.plugin.getConfigString("cmd-help-pooloffset", "§b/lg pooloffset <cue|color> <值> §7- lobbygames.admin §6§l[台球位置调整]"));
        ...
        else if (array[0].equalsIgnoreCase("pooloffset")) {
            if (player.hasPermission("lobbygames.admin")) {
                // 查看当前配置
                if (array.length == 1 || (array.length == 2 && array[1].equalsIgnoreCase("view"))) {
                    double cueOffset = this.plugin.getConfig().getDouble("pool.cue-ball-offset", -1);
                    double colorOffset = this.plugin.getConfig().getDouble("pool.color-ball-offset", -1);
                    
                    // 计算实际使用的值
                    double actualCue = (cueOffset == -1) ? 0.0 : cueOffset;
                    double actualColor = (colorOffset == -1) ? 0.3 : colorOffset;
                    
                    player.sendMessage("§3§m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
                    player.sendMessage("§b§l台球位置配置");
                    player.sendMessage("");
                    player.sendMessage("§7配置值：");
                    player.sendMessage("§7  白球偏移：§b" + cueOffset + (cueOffset == -1 ? " §7(默认值)" : ""));
                    player.sendMessage("§7  彩球偏移：§b" + colorOffset + (colorOffset == -1 ? " §7(默认值)" : ""));
                    player.sendMessage("");
                    player.sendMessage("§7实际使用值：");
                    player.sendMessage("§7  小桌(3x4)：白球§b" + actualCue + "§7，彩球§b" + actualColor);
                    player.sendMessage("§7  大桌(3x5)：白球§b" + actualCue + "§7，彩球§b" + actualColor);
                    player.sendMessage("");
                    player.sendMessage("§7默认值说明：");
                    player.sendMessage("§7  统一默认值：白球§b0§7，彩球§b0.3");
                    player.sendMessage("§7  (小桌和大桌使用相同默认值)");
                    player.sendMessage("");
                    player.sendMessage("§7提示：正数=向内，负数=向外，-1=使用默认值");
                    player.sendMessage("§3§m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
                    return true;
                }
                
                if (array.length >= 3) {
                    String ballType = array[1].toLowerCase();
                    if (!ballType.equals("cue") && !ballType.equals("color")) {
                        player.sendMessage("§4用法：§c/pg pooloffset <cue|color> <值>");
                        player.sendMessage("§7或者：§b/pg pooloffset §7(查看当前配置)");
                        player.sendMessage("§7示例：§b/pg pooloffset cue 0.2 §7(调整白球位置)");
                        player.sendMessage("§7示例：§b/pg pooloffset color 0.4 §7(调整彩球位置)");
                        return true;
                    }
                    
                    double offset;
                    try {
                        offset = Double.parseDouble(array[2]);
                    } catch (NumberFormatException e) {
                        player.sendMessage("§4错误：§c'" + array[2] + "' 不是有效的数字！");
                        return true;
                    }
                    
                    // 限制范围在 -2 到 2 之间（-1 表示使用默认值）
                    if (offset < -2 || offset > 2) {
                        player.sendMessage("§4错误：§c偏移值必须在 -2 到 2 之间！");
                        player.sendMessage("§7提示：-1 = 使用默认值，负数 = 向外偏移，正数 = 向内偏移");
                        return true;
                    }
                    
                    String configKey = ballType.equals("cue") ? "pool.cue-ball-offset" : "pool.color-ball-offset";
                    String ballName = ballType.equals("cue") ? "白球" : "彩球";
                    
                    // 更新配置
                    this.plugin.getConfig().set(configKey, offset);
                    this.plugin.saveConfig();
                    
                    player.sendMessage("§a成功！§7" + ballName + "位置偏移已设置为：§b" + offset);
                    player.sendMessage("§7当前配置：");
                    player.sendMessage("§7  白球偏移：§b" + this.plugin.getConfig().getDouble("pool.cue-ball-offset", -1));
                    player.sendMessage("§7  彩球偏移：§b" + this.plugin.getConfig().getDouble("pool.color-ball-offset", -1));
                    player.sendMessage("§e请使用 §b/pg reload §e重新加载配置，或重启游戏以应用更改");
                    player.sendMessage("§7提示：-1 = 默认值（白球0，彩球0.3）");
                } else {
                    player.sendMessage("§4用法：§c/pg pooloffset <cue|color> <值>");
                    player.sendMessage("§7或者：§b/pg pooloffset §7(查看当前配置)");
                    player.sendMessage("§7调整台球位置偏移（数值越大越靠近中心）");
                    player.sendMessage("§7示例：§b/pg pooloffset cue 0.2 §7(调整白球位置)");
                    player.sendMessage("§7示例：§b/pg pooloffset color 0.4 §7(调整彩球位置)");
                    player.sendMessage("§7示例：§b/pg pooloffset cue -1 §7(恢复默认值)");
                }
            } else {
                player.sendMessage(sp);
            }
            ...
            if (player.hasPermission("lobbygames.command")) {
                if (array.length >= 2) {
                    ...
                    if (arena5.getSpawn1() != null) {
                        player.teleportAsync(arena5.getSpawn1());
                    }
                    else {
                        player.teleportAsync(arena5.getLocation1());
                    }
                }
                else {
    ...
    public List<String> onTabComplete(final CommandSender commandSender, final Command command, final String s, final String[] array) {
        ...
        if (commandSender.hasPermission("lobbygames.command")) {
            if (array.length == 1) {
                ...
                if (array[0].equalsIgnoreCase("pooloffset")) {
                    list.add("cue");
                    list.add("color");
                    list.add("view");
                    return list;
