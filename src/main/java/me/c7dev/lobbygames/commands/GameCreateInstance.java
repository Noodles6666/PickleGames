...
public class GameCreateInstance
{
    ...
    public GameCreateInstance(final Player player, final GameType gt, final LobbyGames plugin) {
        ...
        (this.player = player).sendMessage(this.plugin.getConfigString("create-editing-mode", "\n§b你现在处于编辑模式！使用 §n/pg quit§b 退出。"));
        
        // 发送带有可点击链接的步骤1消息
        sendClickableStep(player, 
            "§2步骤 1：§a前往竞技场的 §b" + locationName + "§a 并运行 ",
            "/pg set", "§b/pg set", "点击执行 /pg set");
     ...
     * 发送带有可点击命令链接的消息
     */
    private void sendClickableStep(Player player, String prefix, String command, String linkText, String hoverText) {
        TextComponent message = new TextComponent(prefix);
        
        TextComponent clickableCommand = new TextComponent(linkText);
        clickableCommand.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, command));
        clickableCommand.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text(hoverText)));
        
        message.addExtra(clickableCommand);
        
        // 添加取消链接
        TextComponent separator = new TextComponent(" §7或 ");
        TextComponent cancelLink = new TextComponent("§c[取消]");
        cancelLink.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/pg quit"));
        cancelLink.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text("点击取消创建")));
        
        message.addExtra(separator);
        message.addExtra(cancelLink);
     ...
     * 发送带有保存和取消链接的消息
     */
    private void sendSaveMessage(Player player, String prefix) {
        TextComponent message = new TextComponent(prefix);
        
        TextComponent saveLink = new TextComponent("§b/pg save");
        saveLink.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/pg save"));
        saveLink.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text("点击保存竞技场")));
        
        message.addExtra(saveLink);
        
        TextComponent suffix = new TextComponent("§a 保存此竞技场！");
    ...
    public void playParticles() {
        ...
        new GameTask() {
            public void run() {
                ...
                location.setZ((double)location.getZ());
        ...
        }.runTaskTimer((Plugin)this.plugin, 1L, 5L);
    }
        ...
        switch (this.instr) {
            case 0: {
                if (this.setLocation1(specialLoc2)) {
                    ...
                    if (this.gt == GameType.CLICKER) {
                        this.instr += 2;
                        sendClickableStep(this.player, 
                            "§2步骤 2：§a前往竞技场的 §b出生点§a 并运行 ",
                            "/pg set", "§b/pg set", "点击执行 /pg set");
                    }
                    else {
                        ++this.instr;
                        sendClickableStep(this.player, 
                            "§2步骤 2：§a前往竞技场的 §b右上角§a 并运行 ",
                            "/pg set", "§b/pg set", "点击执行 /pg set");
                    }
                    this.playParticles();
                    break;
            ...
            case 2: {
                ...
                if (this.gt == GameType.SOCCER) {
                    sendClickableStep(this.player, 
                        "§2步骤 4：§a前往 §b红色足球网§a 的 §b右上角§a 并运行 ",
                        "/pg set", "§b/pg set", "点击执行 /pg set");
                    ++this.instr;
                    break;
                }
                ...
                if (!this.gt.isMultiplayer()) {
                    // 发送带有可点击排行榜链接的消息
                    TextComponent leaderboardMsg = new TextComponent("§2位置已设置！§a要为此竞技场添加本地排行榜，使用 ");
                    TextComponent leaderboardLink = new TextComponent("§b/pg set leaderboard");
                    leaderboardLink.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/pg set leaderboard"));
                    leaderboardLink.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new Text("点击设置排行榜位置")));
                    leaderboardMsg.addExtra(leaderboardLink);
                    this.player.spigot().sendMessage(leaderboardMsg);
                }
                sendSaveMessage(this.player, "§c§l重要：§a不要忘记使用 ");
                this.instr = -1;
                break;
            }
            case 3: {
            ...
            case 4: {
                if (this.setSpecialLoc2(specialLoc2)) {
                    if (this.gt != GameType.SOCCER) {
                        ...
                        this.player.sendMessage("§2位置已设置！");
                    }
                    sendSaveMessage(this.player, "§c§l重要：§a不要忘记使用 ");
                    this.instr = -1;
                    break;
            ...
            default: {
                sendSaveMessage(this.player, "§a所有位置已设置，确保使用 ");
                break;
            }
        }
    }
    ...
    public boolean checkCoords(final Location location, final Location location2) {
        ...
        if (this.gt == GameType.SUDOKU) {
            ...
            if (getMinLength(location, location2) != 2 || maxLength != 4) {
                this.player.sendMessage(this.plugin.getConfigString("create-error-pool-size", "§4错误：§c台球必须是 3x5 大小！"));
                return false;
            }
        }
