    ...
    public void place(final Player player, Location lookingBlock) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (o != null) {
                    spawn.getWorld().spawnParticle(Particle.DUST, spawn.getLocation().add(this.val$particle_offset), 20, Connect4.this.particle_square_radius * Connect4.this.particle_offset_x, Connect4.this.particle_square_radius, Connect4.this.particle_square_radius * Connect4.this.particle_offset_z, (Object)o);
                }
                    ...
                    for (int j = 0; j < Connect4.this.spaces[0].length; ++j) {
                        if (Connect4.this.spaces[i][j] != State.EMPTY) {
                            if (Connect4.this.particles_exclude == null || Connect4.this.particles_exclude.getX() != i || Connect4.this.particles_exclude.getY() != j) {
                                ...
                                add.getWorld().spawnParticle(Particle.DUST, add, Connect4.this.particle_count, Connect4.this.particle_square_radius * Connect4.this.particle_offset_x, Connect4.this.particle_square_radius, Connect4.this.particle_square_radius * Connect4.this.particle_offset_z, (Object)dustOptions);
                            }
                        }
                    }
                }
            }
        }.runTaskTimer((Plugin)this.plugin, 1L, 4L);
    }
    ...
    public void onEnd(final GameEndEvent gameEndEvent) {
        if (gameEndEvent.getGame().getGameType() == this.getGameType() && gameEndEvent.getGame().getArena().getID() == this.arena.getID()) {
            ...
            if (this.plugin.getConfig().getBoolean("connect4.reset-on-end")) {
                ...
                }.runLocationTaskLater((Plugin)this.plugin, Connect4.this.getArena().getCenterPixel(), 120L);
            }
        }
    }
