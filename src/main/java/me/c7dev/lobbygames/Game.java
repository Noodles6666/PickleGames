...
public class Game
{
    ...
    private void afkTimer() {
        final double max_seconds = Game.this.plugin.getConfig().getDouble("afk-kick-seconds", 90.0);
        new GameTask() {
            public void run() {
                ...
                // 使用列表副本避免 ConcurrentModificationException
                final List<UUID> playersCopy = new ArrayList<UUID>(Game.this.players);
                for (final UUID key : playersCopy) {
                    double n;
                    ...
                    if ((currentTimeMillis - n) / 1000.0 > max_seconds) {
        ...
        }.runTaskTimer(this.plugin, 1L, 60L);
    }
    ...
    public void preparePlayer(final Player player) {
        Bukkit.getLogger().info("[Game Debug] preparePlayer called for " + player.getName());
        if (this.inventories.containsKey(player.getUniqueId())) {
            Bukkit.getLogger().warning("[Game Debug] Player " + player.getName() + " already has inventory saved");
            return;
        }
        Bukkit.getLogger().info("[Game Debug] Saving inventory and clearing for " + player.getName());
        this.inventories.put(player.getUniqueId(), player.getInventory().getContents());
        player.getInventory().clear();
        this.p_gm.put(player.getUniqueId(), player.getGameMode());
        if (this.gt.isFlyDisabled()) {
        ...
        Bukkit.getLogger().info("[Game Debug] preparePlayer completed for " + player.getName());
    }
    
    public void returnPlayerInv(final Player player) {
        Bukkit.getLogger().info("[Game Debug] returnPlayerInv called for " + player.getName());
        if (player.isOnline()) {
            if (this.inventories.containsKey(player.getUniqueId())) {
                Bukkit.getLogger().info("[Game Debug] Restoring inventory for " + player.getName());
            ...
            if (this.p_gm.containsKey(player.getUniqueId())) {
                Bukkit.getLogger().info("[Game Debug] Restoring game mode for " + player.getName());
                player.setGameMode((GameMode)this.p_gm.get(player.getUniqueId()));
            }
            if (this.prev_fly.containsKey(player.getUniqueId())) {
                Bukkit.getLogger().info("[Game Debug] Restoring flight status for " + player.getName());
                player.setAllowFlight((boolean)this.prev_fly.get(player.getUniqueId()));
                this.prev_fly.remove(player.getUniqueId());
            }
            Bukkit.getLogger().info("[Game Debug] returnPlayerInv completed for " + player.getName());
        } else {
            Bukkit.getLogger().warning("[Game Debug] Player " + player.getName() + " is not online");
        }
    }
    ...
    public void removePlayer(final Player player) {
        Bukkit.getLogger().info("[Game Debug] removePlayer called for " + player.getName());
        if (!this.players.contains(player.getUniqueId())) {
            Bukkit.getLogger().warning("[Game Debug] Player " + player.getName() + " not in players list");
            return;
        }
        if (this.arena.getSpawn1() != null && !this.no_endtp.contains(player.getUniqueId())) {
            Bukkit.getLogger().info("[Game Debug] Teleporting " + player.getName() + " to spawn");
        ...
        if (this.players.size() <= 1 || (this.gt.isMultiplayer() && this.players.size() <= (this.active ? 2 : 1))) {
            Bukkit.getLogger().info("[Game Debug] Not enough players, calling end()");
            this.end();
        }
        else {
            ...
            Bukkit.getLogger().info("[Game Debug] Scheduling entity task for returnPlayerInv");
            // 在 Folia 上，玩家物品栏操作必须在实体任务中执行
            new GameTask() {
                public void run() {
                    Bukkit.getLogger().info("[Game Debug] returnPlayerInv entity task executing for " + player.getName());
                    Game.this.returnPlayerInv(player);
                    Bukkit.getLogger().info("[Game Debug] returnPlayerInv completed for " + player.getName());
                }
            }.runEntityTask((Plugin)this.plugin, player);
            ...
            Bukkit.getLogger().info("[Game Debug] removePlayer completed for " + player.getName());
        }
    }
    ...
    public void end() {
        ...
        SchedulerUtil.runTaskLaterAsynchronously(this.plugin, () -> {
