...
public class Soccer extends Game implements Listener, Spectatable
{
    ...
    public Soccer(final LobbyGames v1, final Arena v2, final Player player, final SoccerBallEntity soccerBallEntity) {
        ...
        if (!v2.isInBoundsXZ(player.getLocation())) {
            ...
            player.teleport(v2.getCenterPixel());
        }
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (this.val$respawn_seconds == 0) {
                    ...
                    Soccer.this.ball = new Ball(Soccer.this.getArena().getCenterPixel().clone().add(0.0, 10.0, 0.0), Soccer.this);
                    return;
                }
                ...
                }.runTaskTimer((Plugin)Soccer.this.getPlugin(), 0L, 20L);
            }
    ...
    private void onJoin(final PlayerJoinLobbyGameEvent playerJoinLobbyGameEvent) {
        ...
        if (!this.getArena().isInBoundsXZ(playerJoinLobbyGameEvent.getPlayer().getLocation())) {
            playerJoinLobbyGameEvent.getPlayer().teleport(this.getArena().getCenterPixel());
        }
        ...
        if (!this.start_countdown && this.getPlayers().size() >= n) {
            ...
            new BukkitRunnable() {
                ...
                public void run() {
                    ...
                    final int n = this.val$seconds - this.c;
                    if (n == 0) {
                        this.cancel();
                        ...
                        final Iterator<UUID> iterator2 = players.iterator();
                        while (iterator2.hasNext()) {
                            final Player player2 = Bukkit.getPlayer((UUID)iterator2.next());
                            if (player2 != null) {
                    ...
                    else if (Soccer.this.getPlayers().size() < n) {
                        this.cancel();
                    ...
                    else if (this.c == 0 || n % 5 == 0) {
                        Soccer.this.broadcast(this.val$format.replaceAll("\\Q%seconds%\\E", "" + n));
                    }
                    ++this.c;
                }
    ...
    private void onMove(final PlayerMoveEvent playerMoveEvent) {
        ...
        int n = ((this.getArena().isInBoundsXZ(playerMoveEvent.getPlayer().getLocation()) || Arena.isInBoundsXZ(playerMoveEvent.getPlayer().getLocation(), this.net1a, this.net1b) || Arena.isInBoundsXZ(playerMoveEvent.getPlayer().getLocation(), this.net2a, this.net2b)) && playerMoveEvent.getPlayer().getWorld().getName().equals(this.getArena().getCenterPixel().getWorld().getName())) ? 1 : 0;
        if (n != 0) {
            final double n2 = playerMoveEvent.getPlayer().getLocation().getY() - this.getArena().getCenterPixel().getY();
            if (n2 > 11.0 || n2 < 0.0) {
                n = 0;
            }
        }
        if (this.containsPlayer(playerMoveEvent.getPlayer())) {
            if (n == 0) {
                if (this.isActive()) {
                    if (this.a.contains(playerMoveEvent.getPlayer().getUniqueId())) {
                        if (this.a.size() == 1) {
                            this.win(false);
                            return;
                        }
    ...
    private void onDoubleJump(final PlayerToggleFlightEvent playerToggleFlightEvent) {
        ...
        new BukkitRunnable() {
            public void run() {
                if (Soccer.this.canStart() && Soccer.this.containsPlayer(playerToggleFlightEvent.getPlayer())) {
                    playerToggleFlightEvent.getPlayer().setAllowFlight(true);
                }
            }
        }.runTaskLater((Plugin)this.getPlugin(), 20L * this.double_jump_delay - 1L);
        this.doubleJump(playerToggleFlightEvent.getPlayer());
    }
