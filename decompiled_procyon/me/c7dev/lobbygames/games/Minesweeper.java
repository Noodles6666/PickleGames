...
public class Minesweeper extends Game implements Listener
{
    ...
    public void start() {
        ...
        if (super.getArena().getSpawn1() != null) {
            ...
            this.getPlayer1().teleport(super.getArena().getSpawn1());
        }
        ...
        new BukkitRunnable() {
            public void run() {
                ...
                Bukkit.getScheduler().scheduleSyncDelayedTask((Plugin)Minesweeper.this.plugin, (Runnable)new Runnable() {
                    @Override
                    public void run() {
    ...
    public void interactBlock(final Player player, final Block block) {
        ...
        if (super.getArena().isInBounds(location)) {
            ...
            if (handItem != null && handItem.getType() == Material.BLAZE_ROD) {
                ...
                this.die();
                if (LobbyGames.SERVER_VERSION >= 12) {
                    this.getPlayer1().spawnParticle(Particle.EXPLOSION_LARGE, this.getPixel(coordinatePair.getX(), coordinatePair.getY()).add(0.0, 3.0, 0.0), 10);
                }
            }
        }
    }
