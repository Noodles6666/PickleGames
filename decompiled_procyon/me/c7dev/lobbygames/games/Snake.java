...
public class Snake extends Game implements Listener
{
    ...
    private void start(final Player player, final LobbyGames lobbyGames) {
        ...
        this.keylistening = true;
        for (int i = 2; i >= 0; --i) {
            final CoordinatePair coordinatePair = new CoordinatePair(this.headstart.getX() - i, this.headstart.getY());
            this.pixels.add(coordinatePair);
            this.getPixel(coordinatePair).getBlock().setType((i == 0) ? this.head_mat : this.body_mat);
        }
        this.createApple();
        ...
        }.runTaskTimer((Plugin)lobbyGames, 0L, 3L);
    ...
    public void reset(final boolean b) {
        if (LobbyGames.SERVER_VERSION <= 12) {
            GameUtils.fill(this.getArena(), Material.AIR, (byte)0, true, Material.valueOf("WOOL"), (byte)15, false);
        }
        else {
            GameUtils.fill(this.getArena(), Material.AIR, (byte)0, true, Material.BLACK_WOOL, (byte)0, false);
        }
        if (b) {
            this.setActive(false);
            new BukkitRunnable() {
